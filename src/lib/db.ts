...
//
// CRITICAL: D1 init is async (getCloudflareContext + PrismaD1). We cannot
// export an async value. The Proxy intercepts every property/method access
// on `db` and resolves the D1 client lazily before forwarding the call.
  ...
  return new Proxy(function () {}, {
    ...
    apply(_target, _thisArg, args) {
      return getDb().then((client) => {
        ...
        if (typeof prop === 'function') return (prop as (...args: unknown[]) => unknown)(...args)
        return prop
      })
    },
    ...
    get(_target, method) {
      ...
      return (...args: unknown[]) =>
        getDb().then((client) => {
          const model = client[parentProp as keyof PrismaClient] as Record<string, (...args: unknown[]) => unknown>
